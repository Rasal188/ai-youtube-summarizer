        ...
        <div className="space-y-8 font-mono">
            <div className="flex items-center justify-between border-b border-[#FDE68A]/20 pb-4">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight text-[#FFF7CC] uppercase flex items-center gap-2">
                        <Terminal className="h-6 w-6 text-[#FDE68A]" />
                        Memory Logs
                    </h2>
                    <p className="text-[#FDE68A]/80 mt-1">{'>'} accessing_past_summaries...</p>
                </div>
                <Button variant="outline" size="sm" onClick={fetchLogs} disabled={isLoading} className="border-[#FDE68A]/30 text-[#FDE68A] hover:bg-[#FDE68A]/10 bg-transparent rounded-none">
            ...
            {isLoading ? (
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {[1, 2, 3].map(i => (
                        <Card key={i} className="animate-pulse bg-[#111111] border-[#FDE68A]/20 rounded-none h-64">
                            <CardHeader className="h-24 bg-[#FDE68A]/5 border-b border-[#FDE68A]/10 relative" />
                        </Card>
                    ))}
                </div>
            ) : logs.length === 0 ? (
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {logs.map((log) => (
                        <Card key={log.id} className="flex flex-col overflow-hidden group bg-[#111111] border-[#FDE68A]/30 rounded-none shadow-[0_0_15px_rgba(253,230,138,0.05)] hover:shadow-[0_0_20px_rgba(253,230,138,0.15)] hover:border-[#FDE68A]/60 transition-all relative">
                            {/* Target brackets */}
                            <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-[#FDE68A] opacity-50 z-20" />
                            <div className="absolute top-0 right-0 w-2 h-2 border-t border-r border-[#FDE68A] opacity-50 z-20" />

                            <div className="aspect-video bg-[#0A0A0A] relative border-b border-[#FDE68A]/20 shrink-0 flex items-center justify-center overflow-hidden">
                                {/* Scanline */}
                                ...
                                <div className="absolute inset-0 bg-gradient-to-br from-[#FDE68A]/10 to-transparent" />

                                <a href={log.video_url} target="_blank" rel="noreferrer" className="absolute inset-0 z-10 hidden group-hover:flex items-center justify-center bg-black/60 text-[#FDE68A] backdrop-blur-sm transition-all border border-[#FDE68A]/50 m-4 shadow-[0_0_15px_#FDE68A]">
                                    <ExternalLink className="h-8 w-8" />
                                </a>
                            </div>
                            <CardHeader className="p-4 pb-2 bg-[#FDE68A]/5 border-b border-[#FDE68A]/10">
                                <CardTitle className="text-base line-clamp-2 leading-tight text-[#FFF7CC] uppercase text-sm tracking-wider">
                                    {log.video_title}
                                </CardTitle>
                                <div className="text-[10px] text-[#FDE68A]/70 mt-2 uppercase tracking-widest">
